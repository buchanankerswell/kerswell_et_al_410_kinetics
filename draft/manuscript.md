...
\end{equation}
  ...
  1. **Reaction step:** Starting from $X^{t}$, integrate Equation \ref{eq:reaction-rate} over the advection time interval $\Delta t$ using a smaller reaction sub-step $\delta t \le \Delta t$ to obtain an intermediate composition $X^\ast$
  2. **Advection step:** Starting from $X^\ast$, solve the transport of material $\left(\vec{u} \cdot \nabla X \right)$ without phase changes over the advection time interval $\Delta t$ to yield the updated composition $X^{t+1}$
...
Rheological contrasts further modulate kinetic effects by controlling slab trajectory and descent rate. Strong slabs ($B$ $\gtrsim$ 6, viscosity contrast ~6--20$\times$) maintain coherence and penetrate sub-horizontally, spending more time within the phase transition zone where olivine transforms more completely. Weaker slabs ($B$ $\lesssim$ 6, viscosity contrast ~2--6$\times$) deform internally and descend vertically, traversing the transition zone quickly with less time for sluggish reactions. This morphological dependence on slab strength is consistent with previous numerical studies without reaction kinetics [@garel2014], indicating that rheological control of slab geometry operates independently of kinetic effects. Thus under equivalent kinetic conditions, stronger slabs experience more slab pull and continuous descent, while weaker slabs accumulate more metastable olivine, amplifying buoyancy forces that may slow or arrest descent. The simulated descent rates (interquartile range 1.3--2.0 cm/yr, excluding fully stagnated cases) are approximately half of the global interquartile range of vertical slab velocities compiled by @lallemand2005 (2.6--4.8 cm/yr). This discrepancy suggests that buoyancy forces from accumulation of metastable olivine may be more pronounced in Earth's mantle compared to our models, as faster descent rates observed in natural subduction zones should amplify metastability if kinetic rates are sluggish.
...
Slab simulations, in contrast, imply that 410 detectability emerges from joint kinetic and rheological influences (Figure \ref{fig:410-structure-comp}). Strong slabs ($B$ $\gtrsim$ 6, viscosity contrast ~6--20$\times$) descending slowly along sub-horizontal trajectories maximize residence time in the phase transition zone, producing sharp, detectable 410 signals due to near-complete olivine $\Leftrightarrow$ wadsleyite transformation. Weak slabs ($B$ $\lesssim$ 6, viscosity contrast ~2--6$\times$) descending steeply with reduced residence times amplify kinetic inhibition, producing broader reaction fronts. Since tomography shows most slabs descending steeply [@fukao2013], moderate-to-low viscosity contrasts ($B$ $\lesssim$ 6, viscosity contrast ~2--6$\times$) appear common, suggesting kinetic effects should be amplified in many natural subduction zones.
