...
namespace aspect
{
  ...
  {
    template <int dim>
    ReactionKinetics<dim>::ReactionKinetics()
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::initialize()
    {
    ...
    template <int dim>
    bool
    ReactionKinetics<dim>::is_compressible() const
    {
      return true;
    }
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      // Set up reaction kinetics outputs
      std::shared_ptr<ReactionKineticsOutputs<dim>> reaction_kinetics_out = out.template get_additional_output_object<ReactionKineticsOutputs<dim>>();
      ...
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        {
          ...
          if (seismic_out != nullptr && in.requests_property(MaterialProperties::additional_outputs))
            {
              ...
              if (dVp_dT_a_idx != numbers::invalid_unsigned_int && dVp_dT_b_idx != numbers::invalid_unsigned_int)
                {
                  Vps[0] += profile.get_data_component(profile_pos, dVp_dT_a_idx) * (T - T_ref);
                  Vps[1] += profile.get_data_component(profile_pos, dVp_dT_b_idx) * (T - T_ref);
                }
              if (dVs_dT_a_idx != numbers::invalid_unsigned_int && dVs_dT_b_idx != numbers::invalid_unsigned_int)
                {
                  Vss[0] += profile.get_data_component(profile_pos, dVs_dT_a_idx) * (T - T_ref);
                  Vss[1] += profile.get_data_component(profile_pos, dVs_dT_b_idx) * (T - T_ref);
                }
            }
          ...
          if (in.requests_property(MaterialProperties::viscosity))
            {
              ...
              if (reaction_rate_out != nullptr) reaction_kinetics_out->visc_temperature_dependence[q] = visc_temperature_dependence;
            }
          ...
          // Forward reaction (a->b): dX/dt_forward = Z * T * exp(-H_a + PV_a/RT) * (1 - exp(dG/RT)) * (1 - X)
          // Reverse reaction (b->a): dX/dt_reverse = Z * T * exp(-H_a + PV_a/RT) * (1 - exp(-dG/RT)) * X
          // Net reaction rate: dX/dt = dX/dt_forward - dX/dt_reverse
          if (reaction_rate_out != nullptr)
            {
              ...
              // Calculate exponential temperature dependence (dimensionless; same for both directions)
              const double arrhenius = clamp_exp(-(H_a + P * V_a) / (R * T));
              reaction_kinetics_out->arrhenius[q] = arrhenius;
              ...
              {
                if (gibbs >= 0.0) return 0.0;

                const double thermodynamic_driving_force = 1.0 - clamp_exp(gibbs / (R * T));
                if (write_thermo) reaction_kinetics_out->thermodynamic[q] = thermodynamic_driving_force;

                return kinetic_factor * T * arrhenius * thermodynamic_driving_force * saturation;
              };
              ...
              for (unsigned int c = 0; c < this->introspection().n_compositional_fields; ++c)
                if (c != X_idx) reaction_rate_out->reaction_rates[q][c] = 0.0;
            }
        }
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Reaction kinetics");
        {
          ...
          prm.declare_entry("Data file name",
                            "thermodynamic-driving-force-profile-olivine-wadsleyite.txt",
                            Patterns::Anything(),
                            "File that stores the thermodynamic data used for computing the equation of state and reaction kinetics. "
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Reaction kinetics");
      ...
      AssertThrow(this->introspection().n_compositional_fields >= 1,
                  ExcMessage("Reaction kinetics model requires at least one compositional field."));
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::create_additional_named_outputs(
      MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      // Create reaction kinetics outputs
      if (out.template get_additional_output_object<ReactionKineticsOutputs<dim>>() == nullptr)
        {
          const unsigned int n_points = out.n_evaluation_points();
          out.additional_outputs.push_back(std::make_unique<MaterialModel::ReactionKineticsOutputs<dim>>(n_points));
        }
    ...
    template <int dim>
    ReactionKineticsOutputs<dim>::ReactionKineticsOutputs(const unsigned int n_points)
    ...
    template <int dim>
    std::vector<double>
    ReactionKineticsOutputs<dim>::get_nth_output(const unsigned int idx) const
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(
      ReactionKinetics,
      "reaction kinetics",
      "Models a phase transition using reaction kinetics that follow the equation: dX/dt = Z * T * exp(-H_a + PV_a/RT) * (1 - exp(dG/RT)) * (1 - X), "
      "where Q is a user-defined kinetic prefactor, exp(-H_a + PV_a/RT) is an Arrhenius temperature dependence, and (1 - exp(dG/RT)) is the "
      "thermodynamic driving force calculated from internally-consistent thermodynamic data. dG = dG_0 + (P - P_ref)dV - (T - T_ref)dS, where all "
      "thermodynamic parameters are read from a user-specified ASCII data file. "
      "Requires at least one compositional field representing the reacting phase.")
  } // namespace MaterialModel
} // namespace aspect
