...
REMOTE_CONFIGS_DIR="${REMOTE_SIM_DIR}/2d_box/configs"
...
for prm_path in "${CONFIG_LIST[@]}"; do
    ...
    REMOTE_RESULTS_DIR="${REMOTE_SIM_DIR}/2d_box/results/$model_name"
    ...
    for tstep in "${TIMESTEP_LIST[@]}"; do
        ...
        rsync "${RSYNC_OPTS[@]}" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_RESULTS_DIR}/solution/solution-00*${tstep}.pvtu" \
